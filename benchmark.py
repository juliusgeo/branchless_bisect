...
times = []
sizes = [i for i in range(2**20, 2**20+1000)]
times_me = []
times_c = []
times_bl_c = []
# bisect_left
...
for size in sizes:
    ...
    assert len(set([i(test_arr2 if i==bisect_left else test_arr, value) for i in [bisect_left, branchless_bisect_left, bisect_left_c,
                                                                              bl_bisect_left]])) ==1
